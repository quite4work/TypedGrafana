...
describe('positionUtil', () => {
    ...
    it('should position panels', () => {
        ...
        for (const expected of expectedResults) {
            let result = positionUtil(panel, minX, 24, width, height, cursorX, cursorY, 24)
    ...
    it('should position full-width panels', () => {
        ...
        let result = positionUtil(panel, 0, 24, 0, 0, 0, 0, 24)
    ...
    it('should respect the maxX option', () => {
        ...
        let result = positionUtil(panel, 0, 12, 0, 0, 0, 0, 24)
        expect(panel).to.be.positionedAt(0, 0)
        expect(result.currentX).to.eq(0)
        expect(result.currentY).to.eq(6)
    });
    ...
    it("should scale the panel width relative to the column width", () => {
        const panel = new TestPanel().setSize(12, 6)
        var result = positionUtil(panel, 0, 12, 0, 0, 0, 0, 12)
        expect(panel).to.be.positionedAt(0, 0)
        expect(result.currentY).to.eq(0)
        expect(result.currentX).to.eq(6)
    });
});